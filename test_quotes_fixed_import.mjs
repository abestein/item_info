...
async function testQuotesFixedImport() {
    try {
        const config = {
        ...
        console.log('📖 Reading data_import_statements_QUOTES_FIXED.sql...');
        ...
        console.log('🚀 Starting import with quotes-fixed data...');

        let successCount = 0;
        let errorCount = 0;
        const batchSize = 100; // Larger batches since we fixed the main issues
        ...
        for (let batch = 0; batch < totalBatches; batch++) {
            ...
            for (let i = 0; i < batchValues.length; i++) {
                ...
                try {
                    ...
                    // Show more details for first few errors
                    if (errorCount <= 5) {
                        console.error(`  ❌ Error in record ${recordIndex}:`, error.message);
                        console.error(`     Data preview: ${batchValues[i].substring(0, 100)}...`);
                    ...
                    if (errorCount > 200) {
                        console.log('⚠️  Many errors encountered, but continuing...');
                    }
                }
            }
            ...
            if ((batch + 1) % 5 === 0) {
                console.log(`  📊 Progress: ${successCount} success, ${errorCount} errors`);
            }
        }
        ...
        console.log(`📈 Success rate: ${((successCount / valuesArray.length) * 100).toFixed(1)}%`);
        ...
        const previousSuccess = 1807;
        const improvement = successCount - previousSuccess;
        if (improvement > 0) {
            console.log(`\n🚀 Improvement: +${improvement} more records imported than before!`);
        }

        await pool.close();

        return { successCount, errorCount, totalRecords: valuesArray.length };
    } catch (error) {
        console.error('❌ Import process failed:', error.message);
        return { successCount: 0, errorCount: 0, totalRecords: 0 };
    }
}

testQuotesFixedImport();